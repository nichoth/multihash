...
test('creates a hash', t => {
    ...
    getName(pic)
        .then(hashName => {
            ...
            // base64url version
            t.equal(hashName, 'GmuzSvBeEBT5tvt1vhtRkhl1a7V8MkTqCxT4Z4jFz_s.sha256',
...
test('with a base64 image', async t => {
    const hash = await getName(file)
    console.log('from base64', hash)
    t.ok(hash, 'got a hash via base64 image')
    t.equal(hash, 'GmuzSvBeEBT5tvt1vhtRkhl1a7V8MkTqCxT4Z4jFz_s.sha256',
