const { test } = require('tapzero')
const { getHash } = require('../dist/node/index.cjs')

const file = "data:image/png;base64,"

test('get hash of a string', async t => {
    const str = 'a test string'
    const hash = await getHash(str)
    t.ok(hash, 'should create a hash')
})

test('creates a hash from base64 string', async t => {
    const pic = file
    const hashName = await getHash(pic)
    console.log('hash name', hashName)
    t.ok(hashName, 'should create a hash filename')
    t.equal(hashName, 'GmuzSvBeEBT5tvt1vhtRkhl1a7V8MkTqCxT4Z4jFz_s',
        'should return the expected hash, url version')
})

test('with a base64 image', async t => {
    const hash = await getHash(file)
    console.log('from base64', hash)
    t.ok(hash, 'got a hash via base64 image')
    t.equal(hash, 'GmuzSvBeEBT5tvt1vhtRkhl1a7V8MkTqCxT4Z4jFz_s',
        'should return the expected hash')
})
